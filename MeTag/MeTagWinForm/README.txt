...
----------------------------------------
使用Open按钮打开输入的xml文件。
使用Save按钮保存标记好的文件。保存结果又两个，一个是输出文件，一个是包含完整标记信息的方便工具二次读取的中间文件（工具不能直接读取输出文件，目前也不能读取中间文件）
...
选择Tag范围内的一部分后，当前Tag会在右侧粗体显示，同时下拉列表显示当前Tag的附加属性，这个时候再次点击对应快捷键会删除当前Tag
